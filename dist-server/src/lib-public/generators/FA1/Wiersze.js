...
function generateWiersze(faVat) {
    ...
    const isP_PMarzy = Boolean(Number((0, PDF_functions_1.getValue)(faVat.Adnotacje?.P_PMarzy)));
    const faWiersze = (0, PDF_functions_1.getTable)(faVat.FaWiersze?.FaWiersz).map((wiersz) => {
        const marza = (0, Wiersze_1.addMarza)(rodzajFaktury, isP_PMarzy, wiersz);
        if ((0, PDF_functions_1.getValue)(wiersz.P_12)) {
            wiersz.P_12._text = (0, PDF_functions_1.getTStawkaPodatku)((0, PDF_functions_1.getValue)(wiersz.P_12), 1);
        }
        return { ...wiersz, ...marza };
    });
    ...
    const definedHeader2 = [
        ...
        { name: 'DodatkoweInfo', title: 'Dodatkowe informacje', format: common_enum_1.default.Default, width: 'auto' },
        {
            name: 'P_12_Procedura',
            title: 'Procedura',
        ...
        { name: 'Procedura', title: 'Oznaczenia dotyczące procedur', format: common_enum_1.default.Default, width: '*' },
        { name: 'P_6A', title: 'Data dostawy / wykonania', format: common_enum_1.default.Date, width: 'auto' },
    ];
    let content = (0, PDF_functions_1.getContentTable)([...definedHeaderLp, ...definedHeader1, ...definedHeader2], faWiersze, '*');
