...
function generateWarunkiTransakcji(warunkiTransakcji) {
    ...
    if (umowy.length > 0) {
        const tabUmowy = (0, PDF_functions_1.getContentTable)(definedHeaderUmowy, umowy, '*');
        if (tabUmowy.content) {
            Kolumny.umowy = [(0, PDF_functions_1.createSubHeader)('Umowa'), tabUmowy.content];
        }
    }
    if (zamowienia.length > 0) {
        const tabZamowienia = (0, PDF_functions_1.getContentTable)(definedHeaderZamowienia, zamowienia, '*');
