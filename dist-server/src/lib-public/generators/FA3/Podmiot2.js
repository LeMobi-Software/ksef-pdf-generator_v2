...
function generatePodmiot2(podmiot2) {
    ...
    if (podmiot2?.JST) {
        result.push((0, PDF_functions_1.createLabelText)('Faktura dotyczy jednostki podrzędnej JST: ', (0, PDF_functions_1.getValue)(podmiot2?.JST)?.toString().trim() === '1' ? 'TAK' : 'NIE', common_enum_1.default.Default, { marginTop: 8 }));
    }
    if (podmiot2?.GV) {
        result.push((0, PDF_functions_1.createLabelText)('Faktura dotyczy członka grupy GV: ', (0, PDF_functions_1.getValue)(podmiot2?.GV)?.toString().trim() === '1' ? 'TAK' : 'NIE', common_enum_1.default.Default, podmiot2?.JST ? {} : { marginTop: 8 }));
    }
    return result;
}
